...
//    返回受影响的行数
    ...
    public int  updateTicket(long id, Ticket ticket) {
     ...
     //获取票据列表
    //入参: 当期页, 每页的记录数
    public PageInfo<Ticket> findTicketList_by_money(){
        PageHelper.startPage(1, 10);
        List list=ticketDao.findAllTickets_by_money();
        PageInfo<Ticket> pageInfo = new PageInfo<>(list);

        return  pageInfo;  //获取分页对象
    }
    public PageInfo<Ticket> findTicketList_by_type(){
        PageHelper.startPage(1, 10);
        List list=ticketDao.findAllTickets_by_type();
        PageInfo<Ticket> pageInfo = new PageInfo<>(list);
